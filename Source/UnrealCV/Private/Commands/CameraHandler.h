...
/** Supported image serialisation formats. */
enum class EFilenameType : uint8
{
    Png,
    Npy,
    Exr,
...
class FCameraHandler : public FCommandHandler
{
  ...
  private:
    ...
    [[nodiscard]] EFilenameType ParseFilenameType(const FString& Filename);
    ...
    template <class T>
    void SaveData(const TArray<T>& Data, int Width, int Height, const TArray<FString>& Args, FExecStatus& Status);
    ...
    FExecStatus GetCineCamera(const TArray<FString>& Args);
    FExecStatus GetCineCameraEnabled(const TArray<FString>& Args);
    FExecStatus SetCineCameraEnabled(const TArray<FString>& Args);
    FExecStatus SetCineFilmback(const TArray<FString>& Args);
    FExecStatus SetCineLens(const TArray<FString>& Args);
    FExecStatus SetCineFocus(const TArray<FString>& Args);
    FExecStatus SetCineLensSettings(const TArray<FString>& Args);
    FExecStatus SetCineFocusMode(const TArray<FString>& Args);
    FExecStatus SetCineTrackingFocus(const TArray<FString>& Args);
    FExecStatus SetCineCrop(const TArray<FString>& Args);
    FExecStatus SetCineNearClip(const TArray<FString>& Args);
    FExecStatus SetCineExposure(const TArray<FString>& Args);
    FExecStatus GetCineIntrinsics(const TArray<FString>& Args);
