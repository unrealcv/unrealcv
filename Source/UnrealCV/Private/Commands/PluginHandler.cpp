...
DECLARE_CYCLE_STAT(TEXT("FPluginHandler::GetUnrealCVStatus"), STAT_GetUnrealCVStatus, STATGROUP_UnrealCV);
...
FExecStatus FPluginHandler::GetCommandList(const TArray<FString>& Args)
{
    TArray<FString> Commands;
    CommandDispatcher->GetRegisteredCommandTemplates(Commands);
    Commands.Sort();
    return FExecStatus::OK(FString::Join(Commands, TEXT("\n")));
}
...
void FPluginHandler::RegisterCommands()
{
    ...
    Cmd = FDispatcherDelegate::CreateRaw(this, &FPluginHandler::GetCommandList);
    Help = "List registered UnrealCV command templates, one command per line";
    CommandDispatcher->BindCommand(TEXT("vget /unrealcv/commands"), Cmd, Help);
    ...
    CommandDispatcher->BindCommand("vget /level/name",
