...
bool FCommandDispatcher::BindCommand(const FString& ReadableUriTemplate, const FDispatcherDelegate& Command,
                                     const FString& Description)
{
    ...
    UriReadableTemplate.FindOrAdd(UriRegex) = ReadableUriTemplate;
...
void FCommandDispatcher::GetRegisteredCommandTemplates(TArray<FString>& OutTemplates) const
{
    UriReadableTemplate.GenerateValueArray(OutTemplates);
}
...
FExecStatus FCommandDispatcher::Exec(const FString& Uri)
{
    ...
    for (int32 UriIndex = UriList.Num() - 1; UriIndex >= 0; --UriIndex)
    {
        ...
        {
            if (*RegisteredVerb != IncomingVerb)
            {
                continue;
            }
        }
