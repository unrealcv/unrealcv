...
FExecStatus EchoCommand(const TArray<FString>& Args)
{
    if (Args.Num() == 0)
    {
        return FExecStatus::Error(TEXT("no args"));
    }
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FDispatcherBindAndExecTest, "UnrealCV.Server.CommandDispatcher.BindAndExec",
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FDispatcherNoMatchTest, "UnrealCV.Server.CommandDispatcher.NoMatch",
                                 EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FDispatcherMultiArgTest, "UnrealCV.Server.CommandDispatcher.MultiArg",
                                 EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FDispatcherDescriptionTest, "UnrealCV.Server.CommandDispatcher.Description",
                                 EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FDispatcherDescriptionTest::RunTest(const FString& Parameters)
{
    ...
    if (Desc)
    {
        TestEqual(TEXT("Description value"), *Desc, TEXT("My description"));
    }

    return true;
}

IMPLEMENT_SIMPLE_AUTOMATION_TEST(FDispatcherAliasTest, "UnrealCV.Server.CommandDispatcher.Alias",
                                 EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FDispatcherRegisteredTemplatesTest,
                                 "UnrealCV.Server.CommandDispatcher.RegisteredTemplates",
                                 EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FDispatcherRegisteredTemplatesTest::RunTest(const FString& Parameters)
{
    FCommandDispatcher Dispatcher;

    FDispatcherDelegate Cmd;
    Cmd.BindStatic(&EchoCommand);
    Dispatcher.BindCommand(TEXT("vget /test/templates [str]"), Cmd, TEXT("First"));
    Dispatcher.BindCommand(TEXT("vget /test/templates [str]"), Cmd, TEXT("Second"));

    TArray<FString> Templates;
    Dispatcher.GetRegisteredCommandTemplates(Templates);
    TestEqual(TEXT("Only active routes are returned"), Templates.Num(), 2);
    TestTrue(TEXT("Built-in alias route is present"), Templates.Contains(TEXT("vrun [str]")));
    TestTrue(TEXT("Registered route is present"), Templates.Contains(TEXT("vget /test/templates [str]")));
