...
bool FUnrealCVSharedMemoryManager::WriteBytes(const FString& LogicalName, const void* SourceData, int64 NumBytes, FUnrealCVSharedMemoryView& OutView, FString& OutError)
{
	...
	UE_LOG(LogUnrealCV, Warning, TEXT("PERFTRACE|shared_memory_write|logical=%s|bytes=%lld|map_ms=%.3f|memcpy_ms=%.3f|total_ms=%.3f"),
		*LogicalName,
		NumBytes,
		OutView.MapMs,
		OutView.MemcpyMs,
...
bool FUnrealCVSharedMemoryManager::CreateOrResizeRegion(const FString& LogicalName, int64 NumBytes, FUnrealCVSharedMemoryView& OutView, FString& OutError)
{
#if PLATFORM_WINDOWS
	TUniquePtr<FSharedMemoryRegion>* ExistingRegion = Regions.Find(LogicalName);
	if (ExistingRegion && ExistingRegion->IsValid() && (*ExistingRegion)->NumBytes >= NumBytes && (*ExistingRegion)->Data)
	{
	...
	HANDLE MappingHandle = CreateFileMappingW(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, SizeHigh, SizeLow, *PlatformName);
	if (!MappingHandle)
	{
		OutError = FString::Printf(TEXT("CreateFileMappingW failed for '%s' with error %lu"), *PlatformName, GetLastError());
		return false;
	}
