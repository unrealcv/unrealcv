...
@pytest.mark.skipif(
    ver() < (0, 3, 2), reason="eyes_distance is implemented before v0.3.2"
)
def test_camera_distance():
    client.connect(timeout=2)

    for test_distance in [20, 40, 60]:
        res = client.request("vset /action/eyes_distance %d" % test_distance, timeout=5)
        ...
        for _ in range(5):
            client.request("vset /camera/0/rotation %s" % str(random_vec3()), timeout=5)
            actor_loc = Vec3(client.request("vget /actor/location", timeout=5))
            loc1 = Vec3(client.request("vget /camera/0/location", timeout=5))
            loc2 = Vec3(client.request("vget /camera/1/location", timeout=5))
