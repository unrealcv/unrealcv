...
class UE4Automation:
    ...
    def __init__(self, engine):
        ...
        if not os.path.isfile(self.abs_UAT_path):
            message = "Can not found UAT script in the engine folder %s" % self.UE4_dir
            _L.error(message)
    ...
    def install(self, plugin_folder, overwrite=False):
        ...
        _L.info(
            ...
            abs_src_unrealcv_folder,
            abs_tgt_unrealcv_folder,
        )
...
class LinuxBinary(UE4BinaryBase):
    def start(self):
        popen_obj = subprocess.Popen(
            [self.binary_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        )
